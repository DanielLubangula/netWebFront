...
export const ChallengeRoom: React.FC = () => {
  ...
  const { roomId, players: initialPlayers, challengeData, questions: initialQuestions } = location.state || {};
  ...
  useEffect(() => {
    const checkMatchStatus = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/matches/${roomId}`);
  ...
  useEffect(() => {
    ...
    const handleChallengeFinished = (results: { [key: string]: PlayerScore }) => {
      // Utiliser directement les résultats du serveur
  ...
  const finishChallenge = () => {
    // Calculer les scores localement pour le composant PlayersStatus
    const finalScores: { [key: string]: PlayerScore } = {};
    players.forEach(player => {
      finalScores[player.userId] = calculateScore(player.userId);
    });

    // Envoyer les réponses au serveur pour qu'il calcule les scores définitifs
