...
export const ChallengeRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleMatchSnapshot = (snapshot: any) => {
      // console.log('snapbi : ', snapshot)
        return
      // Mettre à jour l'état du match pour les spectateurs
      setPlayers(snapshot.players || players);
      setCurrentQuestionIndex(
        snapshot.currentQuestionIndex ?? currentQuestionIndex
      );
      setPlayerAnswers((prev) => ({
        ...prev,
        ...(snapshot.playerAnswers || {}),
      }));
      setQuestions(snapshot.questions || questions);
      setTimeLeft(snapshot.timeLeft ?? timeLeft);
      setGameStatus(snapshot.status || gameStatus);
    };
    ...
    socket.on("matchSnapshot", handleMatchSnapshot);
    ...
    return () => {
      ...
      socket.off("matchSnapshot", handleMatchSnapshot);
